...
public class DataExporter implements ActionListener, StateHolder {
	...
	public DataExporter(ValueExpression target, ValueExpression type, ValueExpression fileName, ValueExpression pageOnly, ValueExpression selectionOnly, ValueExpression encoding, MethodExpression preProcessor, MethodExpression postProcessor,ValueExpression size) {
		this.target = target;
		this.type = type;
	...
	public void processAction(ActionEvent event){
		...
		try {
			...
			if(!isPageOnly&&!isSelectionOnly){
				if(size!=null){
					try{
						Integer sizeOfOnce = Integer.parseInt((String)size.getValue(elContext));
						if(sizeOfOnce!=null){
							table.setRows(sizeOfOnce);
						}
